...
class CSVModel:
	...
	def getAllRecords(self):
		...
		with open(self.filename, 'r') as fh:
			...
			missingFields = (set(self.fields.keys()) - set(csvreader.fieldnames))
			if len(missingFields) > 0:
				raise Exception("File is missing fields: {}".format(', '.join(missingFields)))
			else:
				records = list(csvreader)

				# Convert text to boolean.
				trues = ('true', 'yes', '1')	# List of TRUE values.
				boolFields = [
					key for key, meta
	...
	def getRecord(self, rownum):
		return self.getAllRecords()[rownum]

	def saveRecord(self, data, rownum=None):
